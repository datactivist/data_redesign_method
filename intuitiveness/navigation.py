"""
Navigation module for the Data Redesign Method.

Implements step-by-step navigation through abstraction levels (L4→L3→L2→L1→L0)
with the following rules:
- L4 is the exclusive entry point
- Users can move up or down one level at a time (vertical only)
- Users cannot return to L4 once they leave (L4 is entry-only)
- Navigation can continue indefinitely
- Users can exit and resume at any point

Implements User Story 5 (Navigate Dataset Hierarchy) from the spec.
"""
...
from .persistence.session_graph import SessionGraph


# =============================================================================
# EXCEPTIONS (from T010 - navigation-specific)
# =============================================================================
...
# =============================================================================
# T007-T013: NavigationTree and NavigationTreeNode (002-ascent-functionality)
# =============================================================================
...
class NavigationTreeNode:
    ...
    decision_description: str = ""  # FR-021: e.g., "Make graph with Indicator, Source"
    output_snapshot: Dict[str, Any] = field(default_factory=dict)  # FR-021

    @property
    def depth(self) -> int:
        """Depth in tree (for UI indentation). Root is depth 0."""
        # This needs to be computed via NavigationTree traversal
        # Stored here as a cached value when needed
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "decision_description": self.decision_description,  # FR-021
            "output_snapshot": self.output_snapshot  # FR-021
        }
...
class NavigationTree:
    ...
    def branch(
        ...
        new_node = NavigationTreeNode(
            ...
            output_snapshot=output_snapshot  # FR-021
        )
    ...
    def restore(self, node_id: str) -> Dataset:
        ...
        # Move current pointer to restored node
...
# =============================================================================
# T045-T053: NavigationSession Class
# =============================================================================
...
class NavigationSession:
    ...
    def get_dimension_options(self, target_level: ComplexityLevel = None) -> List[Dict[str, str]]:
        ...
        try:
            from .ascent.dimensions import DimensionRegistry
    ...
    # -------------------------------------------------------------------------
    # T014: Tree navigation methods (002-ascent-functionality)
    # -------------------------------------------------------------------------
    ...
    def get_cumulative_export(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict mapping level names to output summaries
        """
        from .export.json_export import OutputSummary, CumulativeOutputs
    ...
    def _create_output_summary(self, dataset: Dataset):
        """Create an OutputSummary for a dataset."""
        from .export.json_export import OutputSummary
    ...
    def save_graph(self, filepath: str) -> SessionGraph:
        ...
        if self._use_tree and self._tree:
            ...
            steps = list(self._history.steps)
        else:
            steps = []
